...
[ApiController]
[Route("api/platform/webhook-analytics")]
public class PlatformWebhookAnalyticsController(
    ...
    RbacService rbac) : ControllerBase
{
        ...
        CancellationToken ct = default)
    {
        ...
        {
            var statusSummary = await controlDb.WebhookEvents
                .Where(x => x.ReceivedAtUtc >= from)
                .GroupBy(x => x.Status)
            ...
            return Ok(new
            {
                ...
                from,
                days = safeDays,
                statusSummary = statusSummary.ToDictionary(x => x.key ?? "unknown", x => x.count),
                note = "Pass tenantId to include message status/pricing/origin analytics from MessageEvents."
            });
        }

        var tenant = await controlDb.Tenants.FirstOrDefaultAsync(x => x.Id == tenantId.Value, ct);
        if (tenant is null) return NotFound("Tenant not found.");

        using var tenantDb = SeedData.CreateTenantDbContext(tenant.DataConnectionString);
        var events = tenantDb.MessageEvents.Where(x => x.TenantId == tenantId.Value && x.CreatedAtUtc >= from);

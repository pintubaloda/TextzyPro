...
export async function getPlatformUsers(q = '', ownersOnly = false, includeSuperAdmins = false, effectiveOwnersOnly = false) {
  const qs = new URLSearchParams()
  if (q) qs.set('q', q)
  if (ownersOnly) qs.set('ownersOnly', 'true')
  if (includeSuperAdmins) qs.set('includeSuperAdmins', 'true')
  if (effectiveOwnersOnly) qs.set('effectiveOwnersOnly', 'true')
...
export async function createRazorpayOrder(planCode, billingCycle = 'monthly') {
  return apiPost('/api/billing/razorpay/plan-order', { planCode, billingCycle })
}

export async function createIntegrationRazorpayOrder(slug) {
  return apiPost('/api/billing/razorpay/integration-order', { slug })
}
